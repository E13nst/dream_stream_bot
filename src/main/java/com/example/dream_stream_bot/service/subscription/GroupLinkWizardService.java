...
public class GroupLinkWizardService {
    ...
    public List<OutgoingMessage> onPickTariff(Long privateChatId, BotEntity bot, UserEntity user, long tariffId) {
        ...
        InlineKeyboardMarkup kb = InlineKeyboardMarkup.builder()
                ...
                .build();
        String text = """
                Свяжите бота с вашей группой одним из способов:

                Способ 1 — переслать сообщение:
                Добавьте @%s в вашу группу. Затем перешлите сюда любое сообщение из этой группы.

                Способ 2 — если пересылка запрещена:
                Нажмите кнопку ниже, выберите группу в появившемся окне Telegram."""
                .formatted(uname);
        return List.of(OutgoingMessage.builder()
                .chatId(privateChatId)
                .text(text.strip())
